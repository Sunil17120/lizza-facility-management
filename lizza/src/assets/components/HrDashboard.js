...
import { UserCheck, Shirt, PackagePlus, ShieldAlert, FileText, CheckCircle, RefreshCw, Users, Eye, ChevronRight } from 'lucide-react';
...
const HrDashboard = () => {
    ...
    const fetchData = async () => {
        setIsSyncing(true);
        const [pendRes, invRes, staffRes] = await Promise.all([
            fetch(`${API_BASE_URL}/api/hr/pending-approvals?hr_email=${hrEmail}`),
            fetch(`${API_BASE_URL}/api/hr/inventory`),
            fetch(`${API_BASE_URL}/api/admin/employees?admin_email=${hrEmail}`) // HR can reuse this to see all staff
        ]);
    ...
    const handleVerify = async (email) => {
        if (!window.confirm("Approve this employee and generate LFM ID?")) return;
        const res = await fetch(`${API_BASE_URL}/api/hr/verify-employee`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ target_email: email, hr_email: hrEmail })
        });
        if (res.ok) {
    ...
    const handlePrintProfile = async (userId) => {
        ...
        printWindow.document.write(`
            <html><head><title>Dossier_${emp?.full_name || 'Employee'}</title>
              ...
              <h3 class="section-header">6. Education History</h3>${eduHtml}
              <h3 class="section-header">7. Prior Work Experience</h3>${expHtml}
              <h3 class="section-header">8. Family Details</h3>${famHtml}
              <h3 class="section-header">9. Reference Details</h3>${refHtml}
              ...
              <script>window.onload = function() { setTimeout(() => { window.print(); window.close(); }, 1500); };</script>
            </body></html>
        `);
                ...
                <Tabs defaultActiveKey="approvals" className="custom-pill-tabs mb-4 slide-up">
                    <Tab eventKey="approvals" title={`Pending Setup (${pending.length})`}>
                        <Row className="g-3 mt-2">
                            {pending.length === 0 ? <Col xs={12}><div className="text-center text-muted p-5 bg-white rounded-4 shadow-sm">No pending approvals.</div></Col> :
                                pending.map(p => (
                                        ...
                                        <Card.Body className="p-4 d-flex flex-column">
                                            <div className="d-flex justify-content-between align-items-start mb-3">
                                                <div>
                                                    ...
                                                    <Badge bg="secondary" className="mb-2">{p.department} - {p.designation}</Badge>
                                            ...
                                            <div className="bg-light p-3 rounded-4 mb-4">
                                                <div className="small fw-bold text-primary mb-1">Requested Uniform Kit</div>
                                            ...
                                            <div className="d-flex gap-2 mt-auto">
                                                ...
                                                <Button variant="success" className="flex-grow-1 rounded-pill fw-bold shadow-sm active-scale" onClick={() => handleVerify(p.email)}>
                                                    Verify & Generate ID
                                                </Button>
                                            </div>
                                        </Card.Body>
                    ...
                    <Tab eventKey="directory" title="Active Staff Directory">
                        <Row className="g-3 mt-2">
                            {activeStaff.length === 0 ? <Col xs={12}><div className="text-center text-muted p-5 bg-white rounded-4 shadow-sm">No verified staff found.</div></Col> :
                                activeStaff.map(emp => (
                                <Col xs={12} md={6} lg={4} key={emp.id}>
                                    <Card className="glass-card h-100 border-start border-4 border-success">
                                        <Card.Body className="p-4 d-flex flex-column">
                                            <div className="d-flex align-items-center mb-3">
                                                <img src={emp.profile_photo_path || "https://via.placeholder.com/150"} alt="Profile" className="rounded-circle shadow-sm me-3" style={{width: '50px', height: '50px', objectFit: 'cover'}} />
                                                <div>
                                                    <h6 className="fw-bold mb-0">{emp.full_name}</h6>
                                            ...
                                            <div className="d-flex flex-column gap-2 mt-auto">
                                                ...
                                                <Button variant="primary" size="sm" className="rounded-pill fw-bold shadow-sm d-flex align-items-center justify-content-center active-scale" onClick={() => { setSelectedUserForIssue(emp); setIssueModal(true); }}>
                                                    <Shirt size={16} className="me-2"/> Issue Uniform Kit
                                                </Button>
                                            </div>
                                        </Card.Body>
                                    </Card>
                            ...
                            <Col xs={12} lg={8}>
                                <Card className="glass-card border-0">
                                    <Card.Header className="bg-white py-4 d-flex justify-content-between align-items-center border-bottom-0">
                                        ...
                                        <Button variant="primary" className="rounded-pill fw-bold shadow-sm px-4 active-scale" onClick={() => setShowStockModal(true)}>+ Add Stock</Button>
                                    </Card.Header>
                                            ...
                                            <div className="table-responsive">
                                                <table className="table table-hover align-middle mb-0">
                                                    <thead className="table-light text-muted small text-uppercase">
                                                        <tr><th className="ps-4">Item Type</th><th>Size</th><th className="text-end pe-4">Units Remaining</th></tr>
                                                    </thead>
                                                    <tbody>
                                                        {inventory.map(inv => (
                                                            <tr key={inv.id}>
                                                                <td className="ps-4 py-3 fw-bold text-dark">{inv.item_category}</td>
                                                                <td><Badge bg="dark" className="px-3 py-2 rounded-pill">{inv.size}</Badge></td>
                                                                <td className="text-end pe-4">
                                                                    <Badge bg={inv.quantity > 10 ? "success" : inv.quantity > 0 ? "warning" : "danger"} className="fs-6 rounded-circle p-3 shadow-sm" style={{width: '40px', height: '40px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center'}}>
                                                                        {inv.quantity}
                                                                    </Badge>
                                                                </td>
                                                            </tr>
                                ...
                                <Card className="glass-card bg-primary bg-opacity-10 border-0">
                                    <Card.Body className="p-4">
                                        ...
                                        <div className="bg-white p-3 rounded-4 shadow-sm small text-dark">
                                            <strong>3. Auditing:</strong> Every deduction logs your HR email automatically for security audits.
                                        </div>
                                    </Card.Body>
                                </Card>
                            </Col>
