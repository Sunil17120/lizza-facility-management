...
# Added TaskAssignment here
from database import engine, SessionLocal, User, EmployeeLocation, OfficeLocation, SiteVisit, SiteStay, ShiftLog, FieldOfficerRoute, Attendance, TaskAssignment, init_db, cipher

app = FastAPI()

# Relaxed CORS for seamless Vercel integration and preview deployments
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"], 
...
@app.get("/")
def health_check():
    return {"status": "online", "message": "Lizza Facility Backend is operational", "version": "3.0-tasks-enabled"}
...
async def get_address_from_coords(lat: float, lng: float) -> str:
    ...
    async with httpx.AsyncClient() as client:
        ...
        data = response.json()
        if isinstance(data, dict): return data.get("display_name", "Address not found")
...
def day_shift_action(payload: dict, db: Session = Depends(get_db)):
    ...
    if action == "START":
        if existing_today_shift: raise HTTPException(400, "You have already completed a shift today. Only one shift per day is allowed.")
        if not shift:
...
async def record_location_ping(payload: dict, db: Session = Depends(get_db)):
    ...
    new_route = FieldOfficerRoute(user_id=user.id, shift_id=active_shift.shift_id, latitude=payload.get("lat"), longitude=payload.get("lon"), activity_state=payload.get("activity_state", "TRAVELING"), ping_timestamp=ping_time)
    db.add(new_route)
    db.commit()
    return {"status": "success"}
...
async def submit_task(request: Request, db: Session = Depends(get_db)):
    ...
    # Mirror completion into SiteVisit so it shows in standard reports
    remarks_arr = []
    photos_arr = []
    for item in comp_data:
    ...
    active_stay = db.query(SiteStay).filter(
        SiteStay.officer_id == task.officer_id, 
        SiteStay.location_id == task.location_id, 
    ...
    db.add(SiteVisit(
        ...
        purpose="Assigned Task Completion",
        remarks=json.dumps(remarks_arr),
        photo_path=",".join(photos_arr),
            ...
            if emp.is_present and r:
                ...
                if coords:
                    try:
                        ...
                        emp_data["lat"], emp_data["lon"] = float(lat), float(lon)
                    except: pass
...
async def add_employee(first_name: str = Form(...), last_name: str = Form(...), phone_number: str = Form(...), personal_email: str = Form(...), dob: str = Form(...), designation: str = Form(...), kyc_mode: str = Form(...), userType: str = Form("employee"), gender: Optional[str] = Form(None), marital_status: Optional[str] = Form(None), identity_mark: Optional[str] = Form(None), father_name: Optional[str] = Form(None), mother_name: Optional[str] = Form(None), blood_group: Optional[str] = Form(None), height: Optional[str] = Form(None), caste: Optional[str] = Form(None), category: Optional[str] = Form(None), religion: Optional[str] = Form(None), nationality: Optional[str] = Form(None), medical_remarks: Optional[str] = Form(None), unit_name: Optional[str] = Form(None), perm_address: Optional[str] = Form(None), perm_state: Optional[str] = Form(None), perm_pin: Optional[str] = Form(None), perm_mobile: Optional[str] = Form(None), temp_address: Optional[str] = Form(None), temp_state: Optional[str] = Form(None), temp_pin: Optional[str] = Form(None), temp_mobile: Optional[str] = Form(None), languages_json: Optional[str] = Form(None), education_json: Optional[str] = Form(None), experience_json: Optional[str] = Form(None), family_json: Optional[str] = Form(None), references_json: Optional[str] = Form(None), bank_name: Optional[str] = Form(None), account_number: Optional[str] = Form(None), ifsc_code: Optional[str] = Form(None), aadhar_number: Optional[str] = Form(None), pan_number: Optional[str] = Form(None), voter_id: Optional[str] = Form(None), driving_licence: Optional[str] = Form(None), passport_no: Optional[str] = Form(None), department: Optional[str] = Form("Operations"), manager_id: Optional[int] = Form(None), location_id: Optional[int] = Form(None), shift_start: Optional[str] = Form(None), shift_end: Optional[str] = Form(None), profile_photo: Optional[UploadFile] = File(None), aadhar_photo: Optional[UploadFile] = File(None), pan_photo: Optional[UploadFile] = File(None), voter_photo: Optional[UploadFile] = File(None), dl_photo: Optional[UploadFile] = File(None), passport_photo: Optional[UploadFile] = File(None), fingerprints_left: Optional[UploadFile] = File(None), fingerprints_right: Optional[UploadFile] = File(None), bank_passbook: Optional[UploadFile] = File(None), extra_files: Optional[List[UploadFile]] = File(None), extra_docs_info: Optional[str] = Form(None), db: Session = Depends(get_db)):
    ...
    new_user = User(
        ...
        height=height, caste=caste, category=category, religion=religion, nationality=nationality, medical_remarks=medical_remarks, unit_name=unit_name, designation=designation, department=department, kyc_mode=kyc_mode,
